...
class MaestroEnvironment(object):
    ...
    def __init__(self, model_defs, finish_reward=100, dim_size=6,n_action_steps=2, resource_size=2,  dataflow="dla"):
        ...
        path = "/usr/scratch/felix/my_code/HW_optimizer_rnn_result_history/maestro_his/maestro/"
        if os.path.exists(path) is False:
            path = "/home/felix/Documents/2019summer/HW_optimizer_rnn_result_history/maestro_his/maestro/"
            if os.path.exists(path) is False:
                path = "/Users/chuchu/Documents/gt_local/HW_optimizer_rnn_result_history/maestro_his/maestro/"
        maestro = path + "my_maestro"
    ...
    def get_one_sample(self, action_space, n_action=12 ):
        guess = np.random.randint(0, 12, (len(self.model_defs), self.n_action_steps))
        sol = [[action_space[0][g[0]], action_space[1][g[1]]] for g in guess]
        reward, total_constraint = self.exterior_search(sol)
        return reward, total_constraint, sol
    ...
    def exterior_search(self, actions, action_size=2):
        ...
        for i in range(len(actions)):
            action = actions[i]
            maestro_state = np.concatenate((self.model_defs[i], action))
            feedback = self.oberserve_maestro(maestro_state)
            if feedback is None:
                return None, float("Inf")
            reward, constraint = feedback
            if not reward :
                return None, float("Inf")
                # return float("-inf")
